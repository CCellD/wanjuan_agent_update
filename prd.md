...
- 核心模块
  ...
  - 调试预览聊天：右侧聊天窗口，支持用户消息、AI 消息、图片消息、复制/重生成等操作与布局切换（仅最后一组对话可重生成；欢迎语与推荐问题不提供重生成）。
  - 保存：保存配置与未保存拦截（“请先保存再调试”）。
...
- 调试预览聊天
  - 功能：用户输入、AI 模拟回复、图片消息、复制/重生成按钮、布局切换、清空。
  - 输入/输出：输入框文本与附件；输出为消息气泡，滚动到视底。
  - 规则：
    - 悬浮操作：仅显示复制与重生成；不提供删除。
    - 重生成仅允许最后一组用户+AI 对话；欢迎语与推荐问题消息不显示重生成。
...
- 验收标准
  - 修改后不保存无法发送；保存后导航文本即时更新

#### 2.1.1 主页名称/描述/头像编辑（HOME）
- 用户故事
  - 作为应用管理者，我希望在主页即可修改应用名称、描述与头像，确保展示一致。
- 操作点
...
- 操作点
  - 字段：`ai_model`、`ai_temperature`
  - 范围：`temperature ∈ [0,2]`
- 交互流程
  - 调整参数 → 标记未保存 → 点击“保存”写入本地 → 右侧预览后续对话使用最新参数（当前为模拟回复）；未保存状态拦截发送。
- 校验与提示
  - 超出范围自动钳制到边界；非数值输入回退默认值。
  - 保存成功统一提示；参数变更后摘要区即时显示当前选择的模型。
- 验收标准
  - 刷新后仍读取到最新已保存参数；未保存时无法发送，保存后解除拦截。
...
- 交互流程
  - 编辑提示词 → 标记未保存 → 点击“保存” → 右侧依次自动发送欢迎语（若存在）与推荐问题列表，并在推荐列表顶部显示“猜你想问：”。
- 校验与提示
  - `welcome` 可为空；推荐问题按最多 3 条、去首尾空格后渲染，空字符串不展示。
  - 点击任一推荐项会自动以用户身份发送对应问题并生成模拟 AI 回复。
- 验收标准
  - 保存后链式展示欢迎语与推荐列表；推荐点击行为生效；提示词输入框支持高度拖拽且默认高度 110px。
...
- 交互流程
  - 点击“+ 添加”打开弹窗 → 选择/删除条目、设置参数 → 确认后写入存储并标记未保存 → 列表与摘要计数即时更新 → 顶部“保存”后允许右侧发送。
- 校验与提示
  - `top_k` 为正整数且范围建议 1–50；当存在选中条目时必须设置有效 `top_k`。
  - `kb_rerank_model` 可选；`kb_faq_direct` 为布尔开关，影响 FAQ 是否可直出。
  - 删除操作即时更新摘要计数；未设置必填参数在弹窗底部显示错误提示。
- 验收标准
  - 空态隐藏内容区，四卡高度一致；摘要计数与列表渲染准确。
  - 弹窗“确认”后列表刷新并展开；刷新页面后仍显示最新选择与参数。
  - 未保存状态拦截右侧发送；保存后解除拦截并生效。
...
- 交互流程
  - 从工具库选择并点击“添加”→ 默认启用 → 标记未保存 → 列表与“已启用/总数”摘要即时刷新 → 点击“保存”写入存储 → 右侧显示能力入口（预览为模拟）。
- 校验与提示
  - 重复添加忽略；启用/禁用与删除操作即时刷新启用计数。
- 验收标准
  - 空态内容区隐藏以保证四卡等高；启用计数准确显示；刷新页面后仍保留已添加工具与启用状态。
...
- 选中与弹窗细节
  ...
  - 选择与添加
    ...
    - 工具卡片在“添加/已添加”按钮左侧提供“详情”按钮，点击展示该工具的具体信息。
  - 信息与子弹窗
...
- 交互流程
  - 首次开启：无历史配置时弹出设置弹窗，确认后启用并刷新右侧上传入口。
  - 再次开启：沿用上次已保存配置，直接启用并显示相应入口，不弹窗。
  - 设置按钮：任意时刻点击打开弹窗调整数量与类型，确认后立即应用并刷新入口。
  - 关闭开关：立即禁用并隐藏右侧上传入口。
- 校验与提示
  - 数量范围为 1–20；超出时回退到边界值并同步滑条与数字输入。
  - 类型为文档/图片的任意组合；若均未选中，则右侧上传入口均不展示。
  - 弹窗关闭不保存配置；确认后写入并应用。
- 验收标准
  - 首次开启开关弹出设置弹窗，关闭后再次开启沿用历史配置。
  - 设置确认后入口显示/隐藏与数量/类型立即生效；关闭开关隐藏入口。
  - 预览中选择文档/图片能分别产生对应的用户消息。

- 选中与弹窗细节
  - 触发与关闭
    - 点击开关：
      - 关闭→直接关闭并保存（config.html:812-819）。
      - 首次开启（无历史配置）→ 打开弹窗补齐设置（config.html:820-823）。
      - 再次开启（已有历史配置）→ 直接启用并沿用上次设置（config.html:820-823）。
  ...
  - 设置按钮
    - 位置：紧挨开关左侧（config.html:201-203）。
    - 显示：始终显示（由 `renderUploadButtons` 控制，config.html:896）。
    - 点击：无条件打开设置弹窗（config.html:881）。
    - 效果：在弹窗点击“确认”后将启用开关并应用新设置（config.html:859-868）。

#### 2.7 交互体验配置（UX）
- 用户故事
  - 作为首次进入的用户，我需要在打开页面后看到**欢迎语**与**推荐问题**，便于快速开始对话。
  - 作为运营与配置者，我需要可切换**过程可见性**，在需要时展示中间推理信息或隐藏以简化界面。
  - 作为正在对话的用户，我希望在每轮**AI回复完成后**得到基于上一轮问答内容的**下一步推荐问题**，便于继续深入提问与探索。
  - 作为维护者，我需要能够分别增删改：欢迎语、推荐问题（最多3条，单项可删除）、过程可见性开关、以及“下一步推荐问题”逻辑开关与展现形式。
- 操作点
  - 字段：`welcome`、`rec1/rec2/rec3`、`visible`、`nextStep`
- 交互流程
  - 点击“保存”后：若设置欢迎语则先发送 AI 欢迎；随后发送推荐问题列表（分行按钮），列表顶部显示“猜你想问：”。
  - 点击任一推荐项：自动发送对应用户消息并生成模拟 AI 回复。
- 校验与提示
  - 推荐问题最多 3 条，按去空格后的非空项渲染；点击推荐项无需输入框焦点。
- 验收标准
  - 保存后链式发送（欢迎→“猜你想问：”→推荐列表）；空态列表隐藏，高度与其他卡一致；点击推荐项行为生效。

- 选中与弹窗细节
  - 触发与关闭
    - 点击“+ 添加”打开交互体验弹窗（config.html:832、config.html:773）。
    - 列表行的“信息/详情”按钮同样打开该弹窗并展示全部具体信息。
...
#### 2.8 调试预览聊天（CHAT）
 - 用户故事
  - 我希望在同页测试问答，包含复制、重生成与布局切换。
 - 操作点
  - 输入框：`.input-text`；消息操作：复制/重生成；布局切换：左右/同侧
 - 交互流程
  - 发送用户消息 → 生成模拟 AI 回复 → 悬浮操作（复制/重生成）可用；点击“清空”后显示占位文案并重置输入。
 - 校验与提示
  - 重生成仅允许最后一组用户+AI；欢迎语与推荐问题不显示重生成；空输入不发送；复制失败静默回退。
 - 验收标准
  - AI 悬浮按钮始终右侧；用户在左右布局时按钮左侧、同侧布局时右侧；仅最后一组允许重生成，欢迎语与推荐问题不显示重生成。

#### 2.9 保存（SAVE）
 - 用户故事
  - 我需要保存当前配置；未保存时应被引导先保存。
 - 操作点
  - 保存按钮：清除 `isDirty`。
 - 交互流程
  - 未保存点击发送 → 弹窗提示并拦截；点击“保存”清除未保存标记后允许发送。
 - 验收标准
  - 文案统一；保存后触发欢迎/推荐链式消息。
...
- 主要界面布局
 ...
 - 关键交互流程
  - 修改即未保存；保存后触发欢迎语/推荐问题；未保存拦截发送。
  - 消息悬浮操作：复制/重生成；布局切换按钮用于同侧/左右布局切换。
...
- 数据实体
  ...
  - AIConfig：`ai_model`、`ai_temperature`
...
- 未保存拦截
  - 修改后直接在右侧发送 → 弹窗提示“请先保存再调试”，不追加消息；点击“保存”后可发送。

- 欢迎与推荐链路
  - 保存后若配置欢迎语与推荐问题 → 右侧依次出现 AI 欢迎消息、标题“猜你想问：”与推荐列表；点击任一推荐项自动发送用户消息并生成模拟回复。
  - 推荐项为空或仅空格 → 不渲染推荐列表；仅欢迎语展示。
  - 交互体验卡片中推荐问题拆分为“推荐问题1/2/3”，删除仅影响对应项，其他项保留。

- 工具调用弹窗
  - 弹窗中点击“添加”将工具加入并默认启用；同一工具再次点击“已添加”按钮取消添加，左侧列表与“已启用/总数”摘要即时刷新。
  - 工具列表中启用/禁用、删除操作即时刷新计数；刷新页面后保留已添加工具与启用状态。
  - 每个工具卡片在“添加/已添加”按钮左侧新增“详情”按钮，点击显示工具的具体信息。

- 文件上传设置
  - 首次开启开关（无历史配置）→ 弹出设置弹窗；确认后启用并根据类型显示右侧入口。
  - 再次开启开关（已有历史配置）→ 直接启用并沿用上次设置，不弹窗。
  - 点击“设置”按钮（⚙）始终打开设置弹窗；确认后应用数量（1–20）与类型（文档/图片）并刷新入口；关闭弹窗不改动设置。
  - 关闭开关立即隐藏右侧上传入口。

- 预览消息操作与布局
  - 复制：复制 AI/用户消息文本到剪贴板。
  - 重生成：仅允许最后一组用户+AI 对话；欢迎语与推荐问题不显示重生成。
  - 布局：AI 悬浮按钮始终右侧；左右布局时用户悬浮按钮左侧，同侧布局时用户悬浮按钮右侧；清空后显示占位文案。

- 卡片高度一致性
  - 空态：知识库、工具调用、文件上传、交互体验四卡在空列表时内容区隐藏以保证高度一致。
...
stateDiagram-v2
  ...
  Dirty --> Clean: 点击保存成功
  Dirty --> Dirty: 继续改动
  Dirty --> BlockSend: 右侧尝试发送
  BlockSend --> Dirty: 弹窗提示“请先保存再调试”
```

- 触发改动的操作范围：模型选择、工具添加/开关/删除、上传配置修改、提示词输入、AI 参数、知识库选择/参数、UX 配置、应用名称/描述。
- 右侧发送逻辑：当 `isDirty=true` 时，调用发送函数直接弹窗并返回。
...
### 8.3 消息复制与重生成
 ...
 - 复制：复制用户或 AI 消息文本到剪贴板。
 - 重生成规则：
   - 仅允许最后一组用户+AI 对话显示“重新生成”。
   - 欢迎语与推荐问题消息不显示“重新生成”。
   - 若当前为 AI 行，依据相邻用户消息文本生成“重新生成的回复”。
...
cases:
  ...
  - id: REGEN_ONLY_LAST_PAIR
    given: 存在多轮用户+AI消息
    when: 悬浮查看操作按钮
    then: 仅最后一组用户与AI显示“重新生成”

  - id: WELCOME_RECS_NO_REGEN
    given: 保存后存在欢迎语与推荐列表
    when: 悬浮查看操作按钮
    then: 欢迎语与推荐消息不显示“重新生成”
```
